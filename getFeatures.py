...
from MDA import build_MDA, build_AE
...
params = read_params('example_params.txt')
...
select_arch = params['select_arch']  # a number 1-4 (see below)
select_nets = params['select_nets']  # a number 1-4 (see below)
epochs = int(params['epochs'])
batch_size = int(params['batch_size'])
nf = float(params['noise_factor'])  # nf > 0 for denoising AE/MDA 0.5
...
arch['mda'] = {}
arch['mda']['drug'] = {}
arch['mda']['drug'] = {1: [4 * 100],
                       2: [4 * 500, 4 * 100, 4 * 500],
                       3: [4 * 500, 4 * 300, 4 * 100, 4 * 300, 4 * 500],
                       4: [4 * 500, 4 * 300, 4 * 200, 4 * 100, 4 * 200, 4 * 300, 4 * 500],
                       5: [4 * 800, 4 * 500, 4 * 300, 4 * 200, 4 * 100, 4 * 200, 4 * 300, 4 * 500, 4 * 800],
                       }
...
for i in select_nets:
    ...
    Net = np.loadtxt('./PPMI_CSV/' + org + '_net_' + str(i) + '.csv', delimiter=',')
...
for a in select_arch:
    ...
    if not Path.isfile(models_path + model_name):
        mid_model, history = build_model(Nets, input_dims, arch[model_type][org][a], nf, 1.0, model_type, epochs,
                                         batch_size)
        # save model
...
for model_name in model_names:
    ...
    np.savetxt('./dataset/'+org + model_type + 'Features.txt', features, delimiter='\t', fmt='%s', newline='\n')
    print("Done!")
